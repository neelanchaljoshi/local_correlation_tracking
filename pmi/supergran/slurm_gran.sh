...
index=$1
year=$2
start = datetime(year, 6, 6, 0, 0, 0) + timedelta(hours=index)
...
env time python -W ignore supergran_single_thread.py $dstart $dstop $dspan $dstep $downsample $interpolate -l debug
