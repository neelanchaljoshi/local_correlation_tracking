...
if downsample:
	patch_size = 18 #Size of the patch in pixels for 2k
...
lngc_out = 0.0
latc_out = 40.0
pixscale_out = 0.1 # degrees/pixel
clngarr, clatarr = get_lng_lat_in_postel(nx_out, ny_out, lngc_out, latc_out, pixscale_out)
nlng = len(clngarr)
nlat = len(clatarr)
xylist = np.zeros(clngarr.shape, dtype = [('clng', clngarr.dtype), ('clat', clatarr.dtype)])
xylist['clng'] = clngarr
xylist['clat'] = clatarr
xylist = [tuple(v) for v in xylist.ravel()]
ijlist = [(i,j) for j in range(nlat) for i in range(nlng)]
npatch = len(xylist)
...
for it,dstart_chunk in enumerate(xdays(dstart, dstop, dspan)):
	...
	for dat in xdays(dstart_chunk, dstart_chunk+dspan, dstep):
		...
		t_ref_track = datetime(2010, 6, 6, 12, 0, 0)
