...
# from from_tan_to_postel import get_lng_lat_in_postel
import h5py
...
def tukey_twoD(width, alpha = 0.4):
...
parser.add_argument('dspan', type=int, help='time span in seconds')
parser.add_argument('dstep', type=int, help='time step in seconds')
...
# %% === config: main ===
## input parameters
dspan = timedelta(seconds = args.dspan)
...
dstep = timedelta(seconds = args.dstep)
nt = (dstop - dstart).total_seconds()/dspan.total_seconds()
logger.info('dstop - dstart in seconds = %f', (dstop - dstart).total_seconds())
logger.info('dspan in seconds = %f', dspan.total_seconds())
logger.info('nt = %f', nt)
...
outfile_folder = f'/data/seismo/joshin/pipeline-test/local_correlation_tracking/pmi/ar_inflow/data/data_{resolution}_to_be_stitched_test_{dstep}/'
os.makedirs(outfile_folder, exist_ok=True)
outfile = outfile_folder + f'{args.start}_nt_{nt}_dspan_{args.dspan}_dstep_{args.dstep}_dt_{cadence_interp}_ar_inflow_2deg_gran_{resolution}.hdf5'
...
## output parameters
## output parameters
clatarr = np.arange(-10, 20, 0.5)
clngarr = np.arange(-20, 20, 0.5)
nlng = len(clngarr)
nlat = len(clatarr)
...
for it,dstart_chunk in enumerate(xdays(dstart, dstop, dspan)):
	...
	for dat in xdays(dstart_chunk, dstart_chunk+dspan, dstep):
		...
		t_ref_track = datetime(2015, 11, 4, 6, 0, 0)
		t_ref_idx = np.where(keys['t_rec'] == t_ref_track.strftime(date_fmt))[0][0]
		...
		for ipatch,(clng,clat) in enumerate(xylist):
			...
			dL_change = 0
			dL = keys['crln_obs'][t_ref_idx] - keys['crln_obs'][ii], keys['crln_obs'][t_ref_idx] - keys['crln_obs'][ii+njump] + get_dL_change(clat, 45), keys['crln_obs'][t_ref_idx] - keys['crln_obs'][ii+2*njump] + get_dL_change(clat, 90), keys['crln_obs'][t_ref_idx] - keys['crln_obs'][ii+3*njump] + get_dL_change(clat, 135)

			# logger.info('clat = %f, clng = %f', clat, clng)
