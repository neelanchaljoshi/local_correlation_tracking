...
#SBATCH --job-name=2015_3600s_900s_ar_inflow_2deg_gran_2k_test_interp
...
index=$1
year=$2
start = datetime(year, 11, 1, 0, 0, 0) + timedelta(hours=index*6)
...
env time python -W ignore ar_inflow_single_thread.py $dstart $dstop $dspan $dstep $downsample $interpolate -l debug
