...
def compute_pixel_scale_fdt(distance_au, base_pixel_scale_arcsec=3.57):
    """
...
# %% Load HMI data
keys = Table.read('/scratch/seismo/joshin/pipeline-test/IterativeLCT/hmi.m_45s/keys_new_swan/keys-2014.fits')
paths = keys['path']
hmi_image = fits.open(paths[0][:-1] + '/magnetogram.fits')[1].data
...
# %% Plot results
fig, ax = plt.subplots(1, 2, figsize=(10, 5))
im0 = ax[0].imshow(hmi_image, cmap='gray', vmin=-50, vmax=50, origin = 'lower')
ax[0].set_title('Original HMI Image')
ax[0].set_xlabel('X [pix]')
ax[0].set_ylabel('Y [pix]')
fig.colorbar(im0, ax=ax[0], fraction=0.046, pad=0.04, label='Gauss')
im1 = ax[1].imshow(fdt_no_blur, cmap='gray', vmin=-50, vmax=50, origin = 'lower')
ax[1].set_title('Simulated FDT at 0.5 AU')
ax[1].set_xlabel('X [pix]')
ax[1].set_ylabel('Y [pix]')
fig.colorbar(im1, ax=ax[1], fraction=0.046, pad=0.04, label='Gauss')
plt.tight_layout()
plt.show()
...
fig, ax = plt.subplots(1, 2, figsize=(10, 5))
im0 = ax[0].imshow(hmi_zoomed, cmap='gray', vmin=-50, vmax=50, origin = 'lower')
ax[0].set_title('Zoomed HMI Image')
ax[0].set_xlabel('X [pix]')
ax[0].set_ylabel('Y [pix]')
fig.colorbar(im0, ax=ax[0], fraction=0.046, pad=0.04)
im1 = ax[1].imshow(fdt_blurred_zoomed, cmap='gray', vmin=-50, vmax=50, origin = 'lower')
ax[1].set_title('Zoomed Simulated FDT Image at 0.5 AU')
ax[1].set_xlabel('X [pix]')
ax[1].set_ylabel('Y [pix]')
...
fig, ax = plt.subplots(1, 2, figsize=(10, 5))
im0 = ax[0].imshow(np.nan_to_num(img_out), cmap='gray', vmin=-100, vmax=100, origin = 'lower')
ax[0].set_title('Simulated FDT Image at 0.5 AU (No Blur)')
fig.colorbar(im0, ax=ax[0], fraction=0.046, pad=0.04)
im1 = ax[1].imshow(img_blurred, cmap='gray', vmin=-100, vmax=100, origin = 'lower')
...
zoom_center = (1028, 1024)  # center of FDT image
zoom_size = 280            # size of zoomed region in FDT
...
fig, ax = plt.subplots(1, 2, figsize=(10, 5))
im0 = ax[0].imshow(np.nan_to_num(hmi_zoomed), cmap='gray', vmin=-100, vmax=100, origin = 'lower')
ax[0].set_title('Zoomed HMI at 1 AU')
fig.colorbar(im0, ax=ax[0], fraction=0.046, pad=0.04)
im1 = ax[1].imshow(np.nan_to_num(img_blurred_zoomed), cmap='gray', vmin=-100, vmax=100, origin = 'lower')
...
max_index = np.unravel_index(np.argmax(ccf_blurred), ccf_blurred.shape)
print("Max index of CCF:", max_index)
x_cut = ccf_blurred[max_index[0], :]
y_cut = ccf_blurred[:, max_index[1]]
fwhm_x = find_fwhm(x_mm, x_cut)
fwhm_y = find_fwhm(y_mm, y_cut)
print(f"FWHM in X: {fwhm_x} Mm")
print(f"FWHM in Y: {fwhm_y} Mm")
# %% Plot it again with the FWHM marked
import numpy as np
import matplotlib.pyplot as plt
