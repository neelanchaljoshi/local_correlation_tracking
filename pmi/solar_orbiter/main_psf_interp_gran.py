...
def compute_airy_radius_pixels(wavelength_m, aperture_m, pixel_scale_arcsec):
    ...
    theta_rad = 1.22 * wavelength_m / aperture_m   # radians
...
def simulate_fdt_from_hmi(hmi_image, psf_rel, pix_hmi=0.5, pix_fdt=3.75,
    ...
    fdt_image = map_coordinates(blurred, [y, x], order=1, mode='reflect')
...
# === config: main ===
clatarr = np.arange(-70, 71, 5)
clngarr = np.arange(-70, 71, 5)
					...
					if keys['isbad'][ii] or keys['isbad'][ii+njump]:
						...
						for itry in range(Ntry):
							try:
								...
								if downsample == 1:
									img1 = simulate_fdt_from_hmi(np.nan_to_num(img1), psf_rel_fdt)
									img2 = simulate_fdt_from_hmi(np.nan_to_num(img2), psf_rel_fdt)
								if interp:
									...
									if downsample == 1:
										img3 = simulate_fdt_from_hmi(np.nan_to_num(img3), psf_rel_fdt)
										img4 = simulate_fdt_from_hmi(np.nan_to_num(img4), psf_rel_fdt)
								# if k < 5:
