...
from zclpy3.remap import from_tan_to_postel, rot_tan_map
...
def get_ux_uy_ellipsoid(ccf_av, grid_len, patch_size, pixel_size, R_sun, ntry = 3):
	...
	for n in range(ntry):
		...
		if coeff_array.shape[0] != val_arr.shape[0]:
			logger.warning('shapes do not match!')
			dx_tot = dy_tot = ux = uy = np.nan
...
def compute_pixel_scale_fdt(distance_au = 0.5, base_pixel_scale_arcsec=3.57):
    """
    ...
    """
    return base_pixel_scale_arcsec

def simulate_fdt_from_hmi(img, keys, idx, dsun_au, psf_fdt):
	wcs_in = WCS(naxis=2)
	wcs_in.wcs.crpix = keys['crpix1'][idx], keys['crpix2'][idx]
	wcs_in.wcs.cdelt = keys['cdelt1'][idx], keys['cdelt2'][idx]
	wcs_in.wcs.crval = keys['crval1'][idx], keys['crval2'][idx]
	wcs_out = WCS(naxis=2)
	...
	rsun_obs_in = keys['rsun_obs'][idx]
	rsun_obs_out = rsun_obs_in * (1.0 / dsun_au)
	dB = 0
	dP = 0
	img_out = rot_tan_map(img, wcs_in, rsun_obs_in, dB, dP,
        nx_out, ny_out, wcs_out, rsun_obs_out,
        interp_method=b'bilinear')
	img_fdt = fftconvolve(np.nan_to_num(img_out), psf_fdt, mode='same')
	return img_fdt

def convert_keys_to_fdt(keys, dsun_au):
	# Convert keys to FDT equivalent
	nx_out = 2048
	ny_out = 2048
	keys_fdt = keys.copy()
	keys_fdt['crpix1'] = 0.5*(1+nx_out)
	keys_fdt['crpix2'] = 0.5*(1+ny_out)
	keys_fdt['cdelt1'] = 3.57
	keys_fdt['cdelt2'] = 3.57
	keys_fdt['rsun_obs'] = keys['rsun_obs'] * (1.0 / dsun_au)
	keys_fdt['crval1'] = 0.0
	keys_fdt['crval2'] = 0.0
	return keys_fdt
...
if downsample:
	...
	resolution = '4k'
	infile_fmt = '/scratch/seismo/joshin/pipeline-test/IterativeLCT/hmi.m_45s/keys_new_swan/keys-%Y.fits'
outfile = f'/data/seismo/joshin/pipeline-test/local_correlation_tracking/pmi/solar_orbiter/lct_mag_fdt/data/data_{resolution}_{dstart.year}/{args.start}_nt_{nt}_dspan_{args.dspan}_dstep_{args.dstep}_dt_{cadence_interp}_diff_rot_15deg_mag_{resolution}.hdf5'
segname = 'magnetogram.fits'
if os.path.exists(outfile):
	logger.info('output file %s exists, exiting', outfile)
else:
...
pixel_size = 0.167 #Degrees per pixel in the remapped image

if downsample:
	patch_size = 90 #Size of the patch in pixels for 2k
	pixel_size = 0.167 #Degrees per pixel in the remapped image for 2k
else:
	patch_size = 168
...
## PSF Parameters for HMI and FDT
...
## output parameters
clatarr = np.arange(-90, 91, 7.5)
clngarr = np.arange(-90, 91, 7.5)
nlng = len(clngarr)
nlat = len(clatarr)
...
for it,dstart_chunk in enumerate(xdays(dstart, dstop, dspan)):
	...
	if downsample:
		keys = convert_keys_to_fdt(keys, dsun_au=0.5)
	...
	for dat in xdays(dstart_chunk, dstart_chunk+dspan, dstep):
		...
		if keys['isbad'][ii] or keys['isbad'][ii+njump]:
			...
			if downsample:
				img1 = simulate_fdt_from_hmi(img1, keys, ii, dsun_au, psf_fdt)
				img2 = simulate_fdt_from_hmi(img2, keys, ii+njump, dsun_au, psf_fdt)
				img3 = simulate_fdt_from_hmi(img3, keys, ii+2*njump, dsun_au, psf_fdt)
				img4 = simulate_fdt_from_hmi(img4, keys, ii+3*njump, dsun_au, psf_fdt)
