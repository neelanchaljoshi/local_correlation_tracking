...
if downsample == 1:
	...
	outfile_fmt = '/data/seismo/joshin/pipeline-test/local_correlation_tracking/pmi/diff_rot/data/%d_dspan_{}_dstep_{}_dt_{}_diff_rot_5deg_mag_2k.hdf5'.format(args.dspan, args.dstep, cadence_interp)
	segname = 'magnetogram.fits'
else:
	infile_fmt = '/scratch/seismo/joshin/pipeline-test/IterativeLCT/hmi.m_45s/keys_new_swan/keys-%Y.fits'
	outfile_fmt = '/data/seismo/joshin/pipeline-test/local_correlation_tracking/pmi/diff_rot/data/%d_dspan_{}_dstep_{}_dt_{}_diff_rot_5deg_mag_4k.hdf5'.format(args.dspan, args.dstep, cadence_interp)
	segname = 'magnetogram.fits'
...
if downsample == 1:
	patch_size = 34 #Size of the patch in pixels for 2k
...
clatarr = np.arange(30, 40, 0.1)
clngarr = np.arange(30, 40, 0.1)
nlng = len(clngarr)
nlat = len(clatarr)
...
for yr in range(ystart,ystop):
	...
	dstart_yr = datetime(yr, 1, 5, 0, 0, 0) if yr == 2010 else datetime(yr, 1, 1, 0, 0, 0)
	dstop_yr = datetime(yr+1, 1, 1, 0, 0, 0)
	# dstart_yr = datetime(yr, args.month_start, 15, 0, 0, 0)
	# dstop_yr = datetime(yr, args.month_start, 15, 2, 0, 0)  # changed here warning
	# if flag == 1:
	...
	for it,dstart_chunk in enumerate(xdays(dstart_yr, dstop_yr, dspan)):
			...
			try:
					...
					if keys['isbad'][ii] or keys['isbad'][ii+njump]:
						...
						for itry in range(Ntry):
							try:
								...
								if downsample == 1:
									img1 = simulate_pmi_from_hmi(np.nan_ton_num(img1), psf_rel)
									img2 = simulate_pmi_from_hmi(np.nan_ton_num(img2), psf_rel)
								if interp:
									...
									if downsample == 1:
										img3 = simulate_pmi_from_hmi(np.nan_ton_num(img3), psf_rel)
										img4 = simulate_pmi_from_hmi(np.nan_ton_num(img4), psf_rel)
								# if k < 5:
				...
				for ipatch,(clng,clat) in enumerate(xylist[lo:hi]):
					...
					if (abs(clat) < 0.1) and (abs(clng) < 0.1) and downsample == 0 and save_ccf == True:
						np.save('/data/seismo/joshin/pipeline-test/pmi_test/supergranular_flow/final_sg_compare/ccfs/mag/{}_{}_{}_{}_{}_{}_4k_mag_interp_no_av.npy'.format(dat.strftime('%Y%m%d_%H%M%S'), 'ccf', dspan, dstep, round(clat, 1), round(clng, 1)), ccf)
					if (abs(clat) < 0.1) and (abs(clng) < 0.1) and downsample == 1 and save_ccf == True:
						np.save('/data/seismo/joshin/pipeline-test/pmi_test/supergranular_flow/final_sg_compare/ccfs/mag/{}_{}_{}_{}_{}_{}_2k_mag_interp_no_av.npy'.format(dat.strftime('%Y%m%d_%H%M%S'), 'ccf', dspan, dstep, round(clat, 1), round(clng, 1)), ccf)
					ccfs[ipatch] += ccf
					nums[ipatch] += 1
				# k = k + 1
				comm.Barrier()
		...
		for ixy, (ccf, (clng,clat)) in enumerate(zip(ccfs, xylist[lo:hi])):
			if (abs(clat) < 0.1) and (abs(clng) < 0.1) and downsample == 0 and save_ccf == True:
				np.save('/data/seismo/joshin/pipeline-test/pmi_test/supergranular_flow/final_sg_compare/ccfs/mag/{}_{}_{}_{}_{}_{}_4k_mag_interp_av.npy'.format(dat.strftime('%Y%m%d_%H%M%S'), 'ccf', dspan, dstep, round(clat, 1), round(clng, 1)), ccf)
			if (abs(clat) < 0.1) and (abs(clng) < 0.1) and downsample == 1 and save_ccf == True:
				np.save('/data/seismo/joshin/pipeline-test/pmi_test/supergranular_flow/final_sg_compare/ccfs/mag/{}_{}_{}_{}_{}_{}_2k_mag_interp_av.npy'.format(dat.strftime('%Y%m%d_%H%M%S'), 'ccf', dspan, dstep, round(clat, 1), round(clng, 1)), ccf)
			if np.isnan(ccf).any():
