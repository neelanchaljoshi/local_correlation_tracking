...
if downsample == 1:
	...
	outfile_fmt = '/data/seismo/joshin/pipeline-test/local_correlation_tracking/pmi/diff_rot/data/%d_dspan_{}_dstep_{}_dt_{}_ccf_widths_5deg_mag_2k.hdf5'.format(args.dspan, args.dstep, cadence_interp)
	segname = 'magnetogram.fits'
else:
	infile_fmt = '/scratch/seismo/joshin/pipeline-test/IterativeLCT/hmi.m_45s/keys_new_swan/keys-%Y.fits'
	outfile_fmt = '/data/seismo/joshin/pipeline-test/local_correlation_tracking/pmi/diff_rot/data/%d_dspan_{}_dstep_{}_dt_{}_ccf_widths_5deg_mag_4k.hdf5'.format(args.dspan, args.dstep, cadence_interp)
	segname = 'magnetogram.fits'
...
def get_ux_uy_ellipsoid(ccf_av, grid_len = 7, patch_size = patch_size, pixel_size = pixel_size, R_sun = 696, ntry = 3):
	...
	for n in range(ntry):
		ym, xm = np.where(ccf_av==ccf_av[2:-2, 2:-2].max())
		xmax = xm[0]
		ymax = ym[0]
...
for yr in range(ystart,ystop):
	...
	dstart_yr = datetime(yr, args.month_start, 12, 0, 0, 0)
	dstop_yr = datetime(yr, args.month_stop, 12, 12, 0, 0)
	...
	for it,dstart_chunk in enumerate(xdays(dstart_yr, dstop_yr, dspan)):
		...
		for ixy, (ccf, (clng,clat)) in enumerate(zip(ccfs, xylist[lo:hi])):
			...
			if np.isnan(ccf).any():
				...
				continue
			_,_,ux1[ixy], uy1[ixy] = get_ux_uy_ellipsoid(np.nan_to_num(ccf), grid_len = 5, ntry= 3)
			# --- get ccf widths ---
