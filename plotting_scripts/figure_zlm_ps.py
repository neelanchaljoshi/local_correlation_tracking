...
# %%
l_h22 = np.array([5, 9, 10, 11, 12, 13, 14, 15])
m_h22 = l_h22 - 1
f_h22 = np.array([-330, -278.9, -257.2, -234.3, -199.2, -198.7, -182.8, -181.4])
domega = 453.1 - 456
f_h22 += m_h22*domega
...
ax[0].plot(m_h22, f_h22, marker='D', color='lightgreen', linestyle='none',
           label=r'Hanson et al. 2020', markersize=6, markeredgecolor='white',
...
im = ax[2].pcolormesh(np.arange(flist_uthe_shifted.shape[1]), frqs_binned, power_hdiv_binned, cmap = 'binary', vmax = 3, rasterized = True)
...
ax[2].plot(m_h22, f_h22, marker='D', color='lightgreen', linestyle='none',
           label=r'Hanson et al. 2020', markersize=6, markeredgecolor='white',
           markeredgewidth=0.5, alpha=0.9)
...
ax[1].set_title(r'Power Spectrum of $u_{\theta}$')
ax[2].set_xlabel(r'm')
im = ax[1].pcolormesh(np.arange(flist_uthe_shifted.shape[1]), frqs_binned, power_uthe_binned, cmap = 'binary', vmax = 3, rasterized = True)
...
ax[1].plot(m_h22, f_h22, marker='D', color='lightgreen', linestyle='none',
           label=r'Hanson et al. 2020', markersize=6, markeredgecolor='white',
           markeredgewidth=0.5, alpha=0.9)
...
ax[2].set_title(r'Power Spectrum of $\nabla \cdot \mathbf{u}_h$')
ax[1].set_xlabel(r'm')
fig.tight_layout()
# fig.savefig('/data/seismo/joshin/pipeline-test/local_correlation_tracking/pdfs/hfr_ps_rvort_uthe_hdiv_new.pdf', bbox_inches = 'tight')
...
# ax[0].plot(M_arr, -2*456/(M_arr+1), 'darkorange',label = r'$\omega = -2\Omega/(m+1)$')
ax[0].plot(M_arr, -0.0093*M_arr**2 + 5.855*M_arr - 7.419, 'lime', label = r'$\omega/2\pi = -0.0093m^2 + 5.855m - 7.419$', alpha = 0.7)
ax[0].plot(M_arr, -6.5*456/(M_arr+1), 'cyan',label = r'$\omega = -6.5\Omega/(m+1)$')
ax[0].set_ylim(-400, 400)
ax[0].set_xlim(0, 34)
ax[0].set_xticks(np.arange(0, 35, 1), minor = True)
...
ax[1].plot(M_arr, -2*456/(M_arr+1), 'darkorange',label = r'$\omega = -2\Omega/(m+1)$')
ax[1].plot(M_arr[M_arr >= 21], -41 + 10.2*(M_arr[M_arr >= 21] - 21), 'b-', label=r'$\omega/2\pi = -41 + 10.2(m-21)$', alpha=0.7)
ax[1].plot(M_arr[M_arr >= 19], -63 - 6.5*(M_arr[M_arr >= 19] - 19), 'magenta', label=r'$\omega/2\pi = -63 - 6.5(m-19)$', alpha=0.7)
ax[1].set_ylim(-400, 400)
ax[1].set_xlim(0, 34)
ax[1].set_xticks(np.arange(0, 35, 1), minor = True)
