...
if downsample == 1:
	...
	outfile_fmt = '/data/seismo/joshin/pipeline-test/pmi_test/supergranular_flow/final_sg_compare/data/%d_{}_dspan_{}_dstep_{}_dt_{}_sg_flows_010_res_2deg_save_ccf_gran_final_2k.hdf5'.format(args.month_start, args.dspan, args.dstep, cadence_interp)
	segname = 'continuum.fits'
else:
	infile_fmt = '/scratch/seismo/joshin/pipeline-test/IterativeLCT/hmi.ic_45s/keys_new_swan/keys-%Y.fits'
	outfile_fmt = '/data/seismo/joshin/pipeline-test/pmi_test/supergranular_flow/final_sg_compare/data/%d_{}_dspan_{}_dstep_{}_dt_{}_sg_flows_010_res_2deg_save_ccf_gran_final_4k.hdf5'.format(args.month_start, args.dspan, args.dstep, cadence_interp)
	segname = 'continuum.fits'
					...
					if keys['isbad'][ii] or keys['isbad'][ii+njump]:
						...
						for itry in range(Ntry):
							try:
								...
								if downsample == 1:
									img1 = simulate_pmi_from_hmi(img1, psf_rel)
									img2 = simulate_pmi_from_hmi(img2, psf_rel)
								if interp:
									...
									if downsample == 1:
										img3 = simulate_pmi_from_hmi(img3, psf_rel)
										img4 = simulate_pmi_from_hmi(img4, psf_rel)
								# if k < 5:
